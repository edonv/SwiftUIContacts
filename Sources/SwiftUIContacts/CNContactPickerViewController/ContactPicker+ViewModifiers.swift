...
extension View where Self == ContactPicker {
    ...
    ///
    /// You can set a value with this method to determine which contact should become selectable, such as `emailAddresses.@count > 0` to enable all the contacts that have an email address to become selectable. If no predicate is set, all contacts become selectable.
    ///
    ...
    ///
    /// This method determines whether a selected contact should be returned (when the predicate evaluates to TRUE), or a default action for the property should be performed (when the predicate evaluates to FALSE). By default the contact picker view controller displays the contact’s detail card when a contact is selected.
    ///
    /// To learn about predicate syntax, see [NSPredicate](https://developer.apple.com/documentation/foundation/nspredicate).
    ...
    ///
    /// This method cannot be used when using ``ContactPicker/init(selection:displayedPropertyKeys:)``.
    ///
    /// This method determines whether a selected contact should be returned (when the predicate evaluates to TRUE), or a default action for the property should be performed (when the predicate evaluates to FALSE). By default the contact picker view controller returns the first selected property of the contact. This predicate is evaluated on the [CNContactProperty](https://developer.apple.com/documentation/contacts/cncontactproperty) property that is being selected, such as `(key == 'emailAddresses') AND (value LIKE '*@apple.com')` to return email address of the contact if the address contains the string “@apple.com”.
    ///
    /// To learn about predicate syntax, see [NSPredicate](https://developer.apple.com/documentation/foundation/nspredicate).
